...
// METODOS ESPECIFICOS
	...
	*/
 	...
 	protected void cresca()
 	{
		...
		for(int i=0; i<=this.qtd-1; i++)
			novaFila[i] = this.fila[(this.inicio+i)%this.fila.length];
	...
	*/
 	public X getElemento() throws Exception
 	{
 		...
 		return  this.fila[this.inicio] instanceof Cloneable?
 				this.meuCloneDeX((X)this.fila[this.inicio]):
 				(X)this.fila[this.inicio];
 	}
	...
	*/
 	public void desenfileire() throws Exception
 	{
		if(this.vazia())
 		...
 		if(this.inicio==this.fila.length-1)
 			this.inicio = 0;
 		else
 			this.inicio++;
...
// MetodoS OBRIGATORIOS
	...
	* @return int : retorna o um numero "unico" para cada Fila diferente
	*/
	public int hashCode()
	{
		int ret = 1;
...
// METODOS OBRIGATORIOS AS VEZES
	...
	*/
	...
	{
		if(modelo==null)
			throw new Exception("Fila nao instanciada!");
